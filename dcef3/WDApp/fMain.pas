...
begin
  ...
  begin
    url := Chrome.Browser.Host.GetDevToolsUrl(True);
...
begin
  Chrome.Load('http://magpcss.org/ceforum/apidocs3');
end;

procedure TMainForm.actDomExecute(Sender: TObject);
begin
  Chrome.browser.SendProcessMessage(PID_RENDERER, TCefProcessMessageRef.New('visitdom'));
end;
...
begin
  ...
  MainForm.Chrome.Browser.MainFrame.LoadString(source, '');
end;

procedure TMainForm.actGetSourceExecute(Sender: TObject);
begin
  Chrome.Browser.MainFrame.GetSourceProc(CallbackGetSource);
end;
...
begin
  Chrome.Browser.MainFrame.GetTextProc(CallbackGetText);
end;
...
begin
  Chrome.Load('https://groups.google.com/forum/?fromgroups#!forum/delphichromiumembedded');
end;
procedure TMainForm.actWDAppExecute(Sender: TObject);
begin
  Chrome.Load('https://github.com/djunny/WDApp');
end;
...
begin
  Result := (b <> nil) and (b.Identifier = Chrome.BrowserId) and ((f = nil) or (f.IsMain));
end;
  ...
  case Msg.message of
    ...
    begin
      ...
      begin
        ...
        begin
          Chrome.Browser.Host.ZoomLevel := Chrome.Browser.Host.ZoomLevel+(i/10);
        end
        else begin
        end;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Splitter1.Top := CDebug.Top-10;
      end;
      actDevToolExecute(Nil);
      CDebug.Top := StatusBar.Top;
    end;
  end
...
begin
  if(Chrome.Browser<>Nil)
    AND(browser.Identifier<>Chrome.Browser.Identifier)then
  begin
...
begin
  ...
  begin
    Chrome.Load('about:blank');
    CDebug.Load('about:blank');
    while CDebug.Browser.IsLoading or Chrome.Browser.IsLoading  do
      Application.ProcessMessages;
  end
  else begin
    Chrome.Browser.Host.CloseBrowser(false);
  end;
  CanClose := True;
end;
...
begin
  ...
  Chrome.OnDownloadUpdated         := crmDownloadUpdated;
  Chrome.OnBeforePopup             := crmBeforePopup;
  Chrome.OnAfterCreated            := crmAfterCreated;
  //create debug
  CDebug                           := TChromium.Create(Self);
  CDebug.Parent                    := self;
  CDebug.DefaultUrl                := 'about:blank';
  CDebug.Visible                   := false;
  CDebug.Align                     := albottom;
